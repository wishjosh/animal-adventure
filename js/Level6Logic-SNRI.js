...
class Level6Manager {
  ...
  static checkBearFeed(x, y, z) {
    ...
    if (dist <= 2.2) {
      // 한꺼번에 여러 도토리를 놓아 즉시 합류하는 것을 막는 쿨다운 (신뢰는 천천히 쌓인다)
      const now = (typeof performance !== 'undefined' ? performance.now() : Date.now());
      if (now - Level6Manager._lastBearFeed < 1500) {
        toast('🐻 웅이가 경계하며 천천히 살핍니다. 잠시 후 도토리를 놓아주세요.');
        return;
      }
      ...
      // 신뢰가 쌓일수록 웅이가 도토리를 놓은 쪽으로 한 걸음 다가온다
      bear.x += (x - bear.x) * 0.4;
      bear.z += (z - bear.z) * 0.4;
      if (bear.group) {
        const by = (typeof getH === 'function') ? getH(Math.round(bear.x), Math.round(bear.z)) + 0.5 : bear.y;
        bear.y = by;
        bear.group.position.set(bear.x, by, bear.z);
      }
