...
## 2026-05-20: 레벨 3 '연결의 평원' 미션 로직 및 연동 구현

연결의 평원(Level 3) 콘텐츠의 미션 제어 로직, 전용 3D 리소스(동물 모델/AI) 연동, 클릭/설치/제거 상호작용 검증, 보스 챌린지(먹이사슬 퍼즐 타워) UI 개발을 모두 완료하고 게임 시스템에 공식 연동함.

### 1. 레벨 3 신규 리소스 및 바이옴 데이터 정의
* **`js/data.js`**:
  - `connection_plains` 바이옴의 영역 및 특성(기본 블록, 사막화 블록 및 스폰 데이터) 등록.
  - 레벨 3 수호대 전용 NPC 대사(노루, 들개, 여우, 독수리 대화 등) 추가.
  - 사체 블록(`carcass`) 데이터 추가 및 수호대 도감 항목(`fox`, `eagle`) 신규 정보 구성.
* **`js/state.js`**:
  - 레벨 3 미션 제어용 상태 변수 `level3_phase`, `level3_conditions` 정의.
  - 글로벌 수호대 데이터에 `fox`, `eagle` 플래그 추가.

### 2. 레벨 3 전용 동물 3D 렌더링 및 행동 AI 구현 (`js/world.js`)
* **노루 (Deer)**: 상호작용 및 애니메이션을 위한 3D 메시 구성.
* **들개 (Wild Dog)**: 지정된 배회 구역(`X:-70, Z:-16` 중심의 7칸 이내)을 머무르며, 설치된 블록(울타리, 바위, 관목 등)에 부딪힐 때 반대각으로 튕겨 나가는 바운스 배회 AI 구현.
* **붉은여우 (Fox)**: 들개 격리 전에는 플레이어와 8칸 이하로 근접할 경우 평원 영역 내에서 반대 방향으로 도망치는 겁쟁이 AI 구현. 들개 격리 완료 후에는 제자리에서 플레이어를 응시(회전각 정렬)하며 안정을 찾도록 구성.
* **독수리 (Eagle)**: 날개짓 메시 애니메이션을 추가하고, 평원 정화 시 높은 고목 꼭대기에 스폰되도록 구성.

### 3. 상호작용 및 시스템 감지 훅 구현 (`js/systems.js` & `js/main.js`)
* **블록 설치/제거 감지**:
  - 울타리, 관목 등을 평원에 세우거나 허물 때 BFS(Breadth-First Search) 알고리즘을 이용해 들개 무리가 다른 보호 동물(노루, 여우)과 분리된 채 외부로 나갈 수 없는 "밀폐된 폐곡선 영역(4칸 이상 120칸 이하)"에 갇혔는지 판정(500ms 디바운스 타이머 처리).
  - 사체 블록(`carcass`)을 삽으로 파낼 때마다 감지하여 3개 제거 시 독수리 합류 조건을 트리거.
* **동물 내려놓기(dropAnimal)**:
  - 안아 든 다친 노루를 🍀 마커가 표시된 풀밭 안전지대(DEER_TARGET)에 정상적으로 내려놓을 때의 범위 감지 로직 적용.
* **클릭 핸들링(handleClick)**:
  - 활성화 도구(구급상자 치료, 여우 먹이 주기)에 따른 전용 상호작용 분기 연동.
  - 가이드 화살표 애니메이션 및 나침반 방향 안내 펄싱 렌더링 루프 추가.

### 4. 먹이사슬 보스 챌린지 및 수호대 UI 구성 (`index.html` & `css/style.css` & `js/ui.js`)
* **사막화 연출**: 노루 구출, 들개 격리, 여우 및 독수리 영입이 완료되면 평원 지형 색상(메마른 모래빛)이 급변하는 사막화 시뮬레이션 연출 및 타워 등장 컷신 트리거.
* **먹이사슬 타워 퍼즐 UI**:
  - HTML 드래그 앤 드롭 API를 활용하여 `풀(생산자) ➡️ 메뚜기(1차) ➡️ 붉은여우(2차) ➡️ 독수리(최상위)`를 올바른 높이의 슬롯에 끌어다 매핑하는 미니게임 보드 개발.
  - 검증 완료 시 평원 색상을 완전히 회복하고, 축하 배너(`level3-clear`)를 팝업함.
* **기타 개발 편의성**:
